...
impl Node {
  fn fetch_id_by_path_for_user(
    ...
    //   limit 1;
    //
    // Sadly CTEs or at least recursive ones are not supported by diesels query
    // builder yet: https://github.com/diesel-rs/diesel/issues/356
    // Also non fixed amount of bind values is not supported yet:
    // https://github.com/diesel-rs/diesel/issues/2103
    // Since nothing is supported to execute this query in a proper way, just
    // load each node one after another along the path.
    ...
    conn.transaction(|| {
      ...
      for part in path.iter() {
        ...
        // Add the node if there is one from a previous iteration step as the
        // parent to the query.
        query = match node_id {
          Some(parent_id) => query.filter(nodes::parent_id.eq(parent_id)),
          None => query,
        };
  ...
  pub fn insert(
    ...
    conn.transaction::<_, BackendError, _>(|| {
      let parent_id = if parent_path.is_empty() {
        ...
        // Load parent node id.
        let parent_id =
          Node::fetch_id_by_path_for_user(&conn, owner_id, &parent_path)?;
        Some(parent_id)
      };
      let new_node_data = NewNode::new(payload, parent_id.as_ref(), owner_id)?;
